...
using namespace iop_manipulator_core_fkie;
	...
	for(int i = 0; i < v.size(); i++) {
		if (v[i].getType() == XmlRpc::XmlRpcValue::TypeString) {
			...
			for(XmlRpc::XmlRpcValue::ValueStruct::iterator iterator = v[i].begin(); iterator != v[i].end(); iterator++) {
				...
				ROS_INFO("  %s: %s", joint_name.c_str(), joint_type.c_str());
				p_joint_names.push_back(joint_name);
			}
		}
	}
...
//  int count_joints = 0;
  ...
  ROS_INFO_NAMED("ManipulatorUrdfReader", "Read configuration parameter from ROS configuration server:");
//  pnh.param("manipulator_count_joints", count_joints, 0);
  pnh.param("robot_description", robot_description, std::string("robot_description"));
  ROS_INFO_NAMED("ManipulatorUrdfReader", "  robot_description: %s", robot_description.c_str());
  pnh.param("manipulator_base_link", manipulator_base_link, std::string("manipulator_base_link"));
  ROS_INFO_NAMED("ManipulatorUrdfReader", "  manipulator_base_link: %s", manipulator_base_link.c_str());
  pnh.param("manipulator_tip_link", manipulator_tip_link, std::string("gripperTurn_link"));
  ROS_INFO_NAMED("ManipulatorUrdfReader", "  manipulator_tip_link: %s", manipulator_tip_link.c_str());
...
//      p_get_euler_angles(origin, oroll, opitch, oyaw);
      ...
      if (joint->getType() == moveit::core::JointModel::PRISMATIC)
      {
        ...
        if (iop_joints.size() > 0)
        {
          if (iop_joints[0].getFieldValue() == 0)
          {
            // add jointoffset to preview revolute joint
            jointoffset += iop_joints[0].getRevoluteJointSpecificationRec()->getJointOffset();
            iop_joints[0].getRevoluteJointSpecificationRec()->setJointOffset(jointoffset);
          }
        }
      ...
      if (iop_joints.size() > 0 and (last_axis.x() != axis.x() or last_axis.y() != axis.y()))
      {
        if (iop_joints[0].getFieldValue() == 0)
        {
          ...
          if (iop_joints[0].getRevoluteJointSpecificationRec()->getJointOffset() > 0)
          {
            twistangle *= -1.0;
          }
          iop_joints[0].getRevoluteJointSpecificationRec()->setTwistAngle(twistangle);
          if (joint->getType() == moveit::core::JointModel::PRISMATIC)
          {
            // add jointoffset to preview revolute joint
            jointoffset += iop_joints[0].getRevoluteJointSpecificationRec()->getJointOffset();
...
//          printf("prismatic joint\n");
  ...
  for (unsigned int i = 0; i < iop_joints.size(); i++)
  {
    if (i == 0)
    {
      ...
      if (iop_joints[i].getFieldValue() == 0)
      {
        RevoluteJoint1OffsetRec rjoint1;
        rjoint1.setJoint1Offset(iop_joints[i].getRevoluteJointSpecificationRec()->getJointOffset());
        if (iop_joints[i].getRevoluteJointSpecificationRec()->isMinValueValid())
          rjoint1.setJoint1MinValue(iop_joints[i].getRevoluteJointSpecificationRec()->getMinValue());
        if (iop_joints[i].getRevoluteJointSpecificationRec()->isMaxValueValid())
          rjoint1.setJoint1MaxValue(iop_joints[i].getRevoluteJointSpecificationRec()->getMaxValue());
        if (iop_joints[i].getRevoluteJointSpecificationRec()->isMaxSpeedValid())
          rjoint1.setJoint1MaxSpeed(iop_joints[i].getRevoluteJointSpecificationRec()->getMaxSpeed());
        if (iop_joints[i].getRevoluteJointSpecificationRec()->isMaxTorqueValid())
          rjoint1.setJoint1MaxTorque(iop_joints[i].getRevoluteJointSpecificationRec()->getMaxTorque());
        fjp->setRevoluteJoint1OffsetRec(rjoint1);
      }
      else
      {
        PrismaticJoint1AngleRec pjoint1;
        pjoint1.setJoint1Angle(iop_joints[i].getPrismaticJointSpecificationRec()->getJointAngle());
        pjoint1.setJoint1MinValue(iop_joints[i].getPrismaticJointSpecificationRec()->getMinValue());
        pjoint1.setJoint1MaxValue(iop_joints[i].getPrismaticJointSpecificationRec()->getMaxValue());
        if (iop_joints[i].getPrismaticJointSpecificationRec()->isMaxSpeedValid())
          pjoint1.setJoint1MaxSpeed(iop_joints[i].getPrismaticJointSpecificationRec()->getMaxSpeed());
        if (iop_joints[i].getPrismaticJointSpecificationRec()->isMaxForceValid())
          pjoint1.setJoint1MaxForce(iop_joints[i].getPrismaticJointSpecificationRec()->getMaxForce());
        fjp->setPrismaticJoint1AngleRec(pjoint1);
        fjp->setFieldValue(1);
      }
    }
    else
      p_manipulator_specification.getJointSpecificationList()->addElement(iop_joints[i]);
  }
  // print configuration
...
// print the configuration
  ...
  {
    ROS_INFO_NAMED("ManipulatorUrdfReader", "  FirstJoint: Revolute");
  ...
  {
    ROS_INFO_NAMED("ManipulatorUrdfReader", "  FirstJoint: Prismatic");
  ...
  for (unsigned int i = 0; i < joints_count; i++)
  {
    ...
    {
      ROS_INFO_NAMED("ManipulatorUrdfReader", "  %d. joint: Revolute", i + 1);
    ...
    {
      ROS_INFO_NAMED("ManipulatorUrdfReader", "  %d. joint: Prismatic", i + 1);
