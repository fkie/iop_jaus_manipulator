	...
	**/
	...
	if (pnh.getParam("origin", origin)) {
		...
		if (scan_result == 6) {
			...
			p_pantilt_specification.setPanTiltCoordinateSysX(x);
			p_pantilt_specification.setPanTiltCoordinateSysY(y);
			p_pantilt_specification.setPanTiltCoordinateSysZ(z);
			tf::Quaternion q = tf::createQuaternionFromRPY(roll, pitch, yaw);
			p_pantilt_specification.setAComponentOfUnitQuaternionQ(q.x());
			p_pantilt_specification.setBComponentOfUnitQuaternionQ(q.y());
			p_pantilt_specification.setCComponentOfUnitQuaternionQ(q.z());
			p_pantilt_specification.setDComponentOfUnitQuaternionQ(q.w());
		} else {
	...
	if (pnh.getParam("joint1", joint1)) {
		...
		if (scan_result == 4) {
			...
			p_pantilt_specification.setJoint1MinValue(min_value);
			p_pantilt_specification.setJoint1MaxValue(max_value);
			p_pantilt_specification.setJoint1MaxSpeed(max_speed);
	...
	if (pnh.getParam("joint2", joint2)) {
		...
		if (scan_result == 4) {
			...
			p_pantilt_specification.setJoint2MinValue(min_value);
			p_pantilt_specification.setJoint2MaxValue(max_value);
			p_pantilt_specification.setJoint2MaxSpeed(max_speed);
		} else if (scan_result == 1) {
...
std::vector<std::string> PantiltCfgReader::getJointNames()
{
	std::vector<std::string> joint_names;
	if (!p_joint1.empty()) {
		joint_names.push_back(p_joint1);
	}
	if (!p_joint2.empty()) {
		joint_names.push_back(p_joint2);
	}
	return joint_names;
}
