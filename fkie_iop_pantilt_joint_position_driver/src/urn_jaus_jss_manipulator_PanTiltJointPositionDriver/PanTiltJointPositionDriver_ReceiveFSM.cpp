...
void PanTiltJointPositionDriver_ReceiveFSM::setupIopConfiguration()
{
    ...
    p_pub_cmd_pos_pan = cfg.create_publisher<std_msgs::msg::Float64>("cmd_pos_pan", 1);
    p_pub_cmd_pos_tilt = cfg.create_publisher<std_msgs::msg::Float64>("cmd_pos_tilt", 1);
    p_pub_cmd_pos_pan32 = cfg.create_publisher<std_msgs::msg::Float32>("cmd_pos_pan32", 1);
    p_pub_cmd_pos_tilt32 = cfg.create_publisher<std_msgs::msg::Float32>("cmd_pos_tilt32", 1);
}
...
void PanTiltJointPositionDriver_ReceiveFSM::sendReportCommandedPanTiltJointPositionsAction(QueryCommandedPanTiltJointPositions msg, Receive::Body::ReceiveRec transportData)
{
    ...
    RCLCPP_DEBUG(logger, "ReportCommandedPanTiltJointPositionsAction (%.2f, %.2f) to %d.%d.%d", p_joint1_cmd_position, p_joint2_cmd_position, subsystem_id, node_id, component_id);
...
void PanTiltJointPositionDriver_ReceiveFSM::setPanTiltJointEffortsAction(SetPanTiltJointPositions msg)
{
    ...
    RCLCPP_DEBUG(logger, "setPanTiltJointEffortAction (%.2f, %.2f)", p_joint1_cmd_position, p_joint2_cmd_position);
    sensor_msgs::msg::JointState cmd_joint_state;
    ...
    if (p_use_posestamped) {
        ...
        p_pub_pos_stamped->publish(cmd_pose);
    }
}
...
void PanTiltJointPositionDriver_ReceiveFSM::stopMotionAction()
{
    ...
    RCLCPP_DEBUG(logger, "stopMotionAction");
...
void PanTiltJointPositionDriver_ReceiveFSM::motion_profile_received(JausAddress reporter, urn_jaus_jss_manipulator_PanTiltMotionProfile::ReportPanTiltMotionProfile profile)
{
    // TODO: handle motion profile
    RCLCPP_DEBUG(logger, "motion_profile received from %s", reporter.str().c_str());
}
